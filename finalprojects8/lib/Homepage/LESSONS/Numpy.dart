...
class Numpy extends StatefulWidget {
  const Numpy({super.key});

  @override
  _NumpyState createState() => _NumpyState();
}
...
class _NumpyState extends State<Numpy> {
  ...
  Future<void> getLessonProgress() async {
    ...
    if (doc.exists) {
      ...
      setState(() {
        isLessonComplete = data['intlessons']?['Numpy'] ?? false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Python Numpy & Runner'),
        centerTitle: true,
      ),
      body: Container(
